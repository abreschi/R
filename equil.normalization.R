...
make_option(c("-c", "--center_only"), action="store_true", help="just center the data without scaling", default=FALSE),
...
equil = function(matr) {
	if (opt$n_iter == 0) {matr = t(scale(t(matr), scale_attr))
		}else{
		for (i in 1:opt$n_iter) {
			matr = t(scale(t(matr), scale_attr))
			matr = scale(matr, scale_attr)
			}
		}
	return(matr)
	}
...
equil = function(matr) {
	if (opt$n_iter == 0) {matr = scale(matr, scale_attr)
		}else{
		for (i in 1:opt$n_iter) {
			matr = scale(matr, scale_attr)
			matr = t(scale(t(matr), scale_attr))
...
# print output
#--------------
output = sprintf('equil.log_%s.pscnt_%s.rowFirst_%s', opt$log, opt$pseudocount, opt$row_first)
