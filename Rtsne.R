...
map_df = data.frame(x=tsne$Y[,1], y=tsne$Y[,2], id=id)
...
if (is.null(opt$color_by)) {
	...
	mapping = modifyList(mapping, aes_string(color=opt$color_by))
}

if (is.null(opt$shape_by)) {
	geom_params$shape = 19
} else {
	mapping = modifyList(mapping, aes_string(shape=opt$shape_by))
}
