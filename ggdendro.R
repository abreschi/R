...
colSide_palette = as.character(read.table(opt$colSide_palette, h=F)$V1)}
...
if (!is.null(opt$col_metadata)) {
	col_mdata = read.table(opt$col_metadata, h=T, sep="\t")
...
dendro_vp_y = colbar_h*length(ColSides) # + col_labels_inches
dendro_vp_x = 0.1
